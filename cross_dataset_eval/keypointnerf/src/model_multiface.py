...
from .multiface_dataset import MultifaceDataset
...
class KeypointNeRFLightningModule(pytorch_lightning.LightningModule):
    ...
    @torch.no_grad()
    def test_multiface(self, data_loader, label='', **kwargs):

        # outname = data_loader.test_split.replace('/', ':')
        dst_dir = os.path.join(self.save_dir, 'test_multiface', label)
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            gt_img = out_nerf['tar_img'].permute(1, 2, 0).cpu().numpy()
            imageio.imwrite(f'{dst_dir}/{name}_{n}.png', (rendered_image*255).astype(np.uint8))
            imageio.imwrite(f'{dst_dir}/{name}_gt_{n}.png', (gt_img*255).astype(np.uint8))
            n += 1

    @torch.no_grad()
    def test_multiface_single(self, batch, label='', **kwargs):

        # outname = data_loader.test_split.replace('/', ':')
        dst_dir = os.path.join(self.save_dir, 'test', label)
        os.makedirs(dst_dir, exist_ok=True)
        self.eval()
