...
from .facescape_dataset import FacescapeDataset
...
def load_json_torch(path):
    with open(path) as f:
        cameras = json.load(f)
    for i in range(len(cameras)):
        cameras[i]['w2cs'] = torch.tensor(np.array(cameras[i]['w2cs'])).float().to('cuda')
        cameras[i]['c2ws'] = torch.tensor(np.array(cameras[i]['c2ws'])).float().to('cuda')
        cameras[i]['intrinsics'] = torch.tensor(np.array(cameras[i]['intrinsics'])).float().to('cuda')
    return cameras
...
class KeypointNeRFLightningModule(pytorch_lightning.LightningModule):
    def __init__(self, cfg: dict, cfg_model: dict):
        ...
        self.dataset = FacescapeDataset
        self.video_dirname = 'val_unseen_id'
        self.images_dirname = 'images'
        self.test_dst_name = 'v3'

        self.model = KeypointNeRF(self.cfg)
        self.znear, self.zfar = 0.6, 2.2
    ...
    @torch.no_grad()
    def test_facescape(self, data_loader, label='', **kwargs):

        dst_dir = os.path.join(self.save_dir, 'test_facescape', label)
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            name = str(batch['index']['frame'][0])
            gt_img = out_nerf['tar_img'].permute(1, 2, 0).cpu().numpy()
            # write source imgs too
            source_imgs = batch['images'][0].detach().cpu().numpy().transpose(0,2,3,1)
            # print(f"source_imgs: {source_imgs.shape}")
            imgout = np.concatenate(((source_imgs[1]*255).astype(np.uint8), \
                                    (source_imgs[2]*255).astype(np.uint8), \
                                    (source_imgs[3]*255).astype(np.uint8), \
                                    (rendered_image*255).astype(np.uint8), \
                                    (gt_img*255).astype(np.uint8)), axis=1)
            imageio.imwrite(f'{dst_dir}/{name}_out.png', imgout)
            
            # imageio.imwrite(f'{dst_dir}/{name}.png', (rendered_image*255).astype(np.uint8))
            # imageio.imwrite(f'{dst_dir}/{name}_gt.png', (gt_img*255).astype(np.uint8))
...
class KeypointNeRF(torch.nn.Module):
    ...
    def query(
        ...
        **kwargs):
        ...
        sp_data.update(cam)
        y = self.sp_encoder(**sp_data) # 计算相对深度
        if y is not None:
            y = y.view(-1, n_views, *y.shape[1:])
        y_all = None

        out, valid, latent_view, latent_fused = self.mlp_geo(y, feat_sampled, out_mask, pix_weight, y_all) # sn和feat_geo结合,输出alpha
        img = tx_data['img']
        ...
        out = torch.cat([out, rgb], -1)  # [_, alpha, rgb]  mlp_geo和mlp_tex结果concat一起
        return out, valid
    ...
    def query_color(self, v, xy, view, n_views, feat_tex, latent_fused, cam, img, out_mask, n_samples):
        ...
        feat_xy = feat_sample(feat_tex, xy)  # (BV, N, feat_leat) 查询对应点的feature
        latent_fused = self.ibr_compress_gfeat(latent_fused)
        rgb_feat = torch.cat((img_xy.view(-1, *img_xy.shape[-2:]), feat_xy, latent_fused), dim=-1) # [位置对应像素，pixel aligned feature， 位置编码Gx]
        ...
        # view dependent color field,输出color，参照IBR的结构
        out = self.mlp_tex(
    ...
    @staticmethod
    def batch_render_pifu_nerf( # 主要的forward函数
        net,
        ...
        **config
        ...
        if net.training:
            ...
            # print(f"msk_coords: {msk_coords.shape}")
            ...
            # print(f"grid_center: {grid_center.shape}")
            # print(f"out_h: {out_h}")
        ...
        mask_z1 = (hit & (z1 > znear_rays)).float()  # 看着是手动定义的nearfar和bbox共同计算出每个采样方向上的near far
        ...
        z = z[None, None, :].expand(*znear_rays.shape[:2], -1)  # (B,h*w,sample_c) 根据near far来决定采样范围
