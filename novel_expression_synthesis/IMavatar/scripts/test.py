...
class TestRunner():
    ...
    def run(self):
        ...
        #     _, model_input_next, _ = next(eval_iterator)
        #     _, model_input_next, _ = next(eval_iterator)
        #     model_pose = [model_input['flame_pose'].clone(), model_input_next['flame_pose'].clone()]
        # for i in np.arange(-1, 1, 0.1):
        #     model_input['flame_pose']=model_pose[0].clone()
        #     model_input['flame_pose'][0][4] = i
        #     print(model_input['flame_pose'][0][4])
        ...
        #     from utils.calculate_metrics import run as cal_metrics
        #     for i in range(0, 12):
        #         out_dir = plot_dir[0].replace('e_11', f'e_{i}')
        #         gt_dir = self.plot_dataset.gt_dir.replace('e_11', f'e_{i}')
        #         cal_metrics(output_dir=out_dir, gt_dir=gt_dir, pred_file_name='rgb_erode_dilate')
        #         cal_metrics(output_dir=out_dir, gt_dir=gt_dir, pred_file_name='rgb_erode_dilate', no_cloth=True)