...
if not os.path.isfile(vid_file):
    print('no video')
    exit()
aud_file = os.path.join('dataset', 'auds', id+'.wav')
if not os.path.isfile(aud_file):
    print('no audio')
    exit()
...
if running_step == 0:
    ...
    extract_wav_cmd = 'cp ' + aud_file + ' ' + wav_file
...
if running_step == 1:
    print('--- Step1: extract images from vids renderme-360 ---')
    cap = cv2.VideoCapture(vid_file)
    frame_num = 0
    total_frame_num = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    # while(True):
    for _ in trange(total_frame_num):
        _, frame = cap.read()
        if frame is None:
            break
        # print(frame.shape)
        if frame.shape[0] == 2048 and frame.shape[1] == 2448:
            # frame = cv2.resize(frame[:,200:-200,:], (450,450), cv2.INTER_LANCZOS4)
            if '094' in vid_file:
                frame = cv2.resize(frame[275:-225,550:-350,:], (450,450), cv2.INTER_LANCZOS4)
            elif '029' in vid_file:
                frame = cv2.resize(frame[275:-225,400:-500,:], (450,450), cv2.INTER_LANCZOS4)
            else:
                frame = cv2.resize(frame[275:-225,450:-450,:], (450,450), cv2.INTER_LANCZOS4)
...
if running_step == 2:
    ...
    ####### Using landmark detector in RenderMe-360's pipeline

        
max_frame_num = 100000
valid_img_ids = []
for i in trange(max_frame_num):
...
if running_step == 3:
    ...
    ####### Using face parsing in RenderMe-360's pipeline
...
if running_step == 4:
    ...
    ####### Using background captured in RenderMe-360's pipeline
...
if running_step == 7:
    ...
    print('id_dir: ', id_dir)
    if 's1_0' in id_dir:
        HeadNeRF_config_file = os.path.join(id_dir, 'HeadNeRF_config_s123456.txt')
        with open(HeadNeRF_config_file, 'w') as file:
            file.write('expname = ' + id + '_head\n')
            file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')

            datadir_list = 'datadir_list = ['
            for suff in ['s1_0', 's2_0', 's3_0', 's4_0', 's5_0', 's6_0']:
                datadir_list += os.path.join(dir_path, 'dataset', id[:-4]+suff+',')
            datadir_list = datadir_list[:-1] + ']\n'
            file.write(datadir_list)
        ...
        ComNeRF_config_file = os.path.join(id_dir, 'TorsoNeRF_config_s123456.txt')
        with open(ComNeRF_config_file, 'w') as file:
            file.write('expname = ' + id + '_com\n')
            file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
        ...
        ComNeRFTest_config_file = os.path.join(id_dir, 'TorsoNeRFTest_config_s123456.txt')
        with open(ComNeRFTest_config_file, 'w') as file:
            file.write('expname = ' + id + '_com\n')
            file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
        ...
        ComNeRFTest_config_file = os.path.join(id_dir, 'TorsoNeRFTest_config_s123456_02.txt')
        with open(ComNeRFTest_config_file, 'w') as file:
            file.write('expname = ' + id + '_com\n')
            file.write('datadir = ' + os.path.join(dir_path, 'dataset', id) + '\n')
            ...
            file.write('far = ' + str(mean_z+0.4) + '\n')
            file.write('with_test = ' + str(1) + '\n')
            file.write('test_pose_file = transforms_train.json' + '\n')
