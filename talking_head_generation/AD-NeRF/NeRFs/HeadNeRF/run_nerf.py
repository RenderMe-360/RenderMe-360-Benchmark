from load_audface import load_audface_data, load_audface_data_multi
import os
import sys
import numpy as np
import imageio
...
def train():
    ...
    if args.dataset_type == 'audface':
        if args.with_test == 1:
            ...
            if args.datadir_list == []: # load data from one video
                images, poses, auds, bc_img, hwfcxy, sample_rects, mouth_rects, i_split = load_audface_data(
                    args.datadir, args.testskip)
            else: # load data from multi videos
                images, poses, auds, bc_img, hwfcxy, sample_rects, mouth_rects, i_split = load_audface_data_multi(
                    args.datadir_list, args.testskip)
