from load_audface import load_audface_data, load_test_data, load_audface_data_multi
import os
import sys
import numpy as np
import imageio
...
def create_nerf(args, ext, dim_aud, device_spec=torch.device('cuda', 0), with_audatt=False):
    ...
    # for testing only
    ckpts = [i for i in ckpts if i.split('/')[-1] != '600000_body.tar']
...
def config_parser():
    ...
    parser.add_argument("--datadir_list", nargs='+', default=[],
                        help='input data directory')
...
def train():
    ...
    if args.with_test == 1:
        ...
        print(args.test_pose_file, args.testskip, args.test_size, args.aud_start)
        print(poses.shape, auds.shape)
        torso_pose = torch.as_tensor(torso_pose).to(device_torso).float()
        com_images = np.zeros(1)
    else:
        if args.datadir_list == []:
            com_images, poses, auds, bc_img, hwfcxy, sample_rects, \
                i_split = load_audface_data(args.datadir, args.testskip)
        else:
            com_images, poses, auds, bc_img, hwfcxy, sample_rects, \
                i_split = load_audface_data_multi(args.datadir_list, args.testskip)
    ...
    print('start, iter ', start, N_iters)

    # start = start + 1 
    start = start + 1 - 600000 # use-pretrained body model
