...
if __name__ == "__main__":
    ...
    for subjects in tqdm([hair_short, hair_long, hair_curls]):
        ...
        for subject in subjects:
            dir_img_pred = os.path.join(result_dir, subject, 'output/train_fixed_view_time_interpolation_15')
            preds_gt = sorted([f for f in os.listdir(dir_img_pred) if f.endswith('.png') and f.startswith('rgb_')])
