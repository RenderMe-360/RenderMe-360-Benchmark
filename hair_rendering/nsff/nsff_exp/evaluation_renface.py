...
def load_novel_pose(basedir,recenter=True, factor=None, width=None, height=None):
    poses_arr = np.load(os.path.join(basedir, 'mv_poses_bounds.npy'))
...
def evaluation():
    ...
    if not args.datadir.startswith('/mnt'):
        abspath = os.path.dirname(os.path.abspath(__file__))
        args.datadir = f'{abspath}/../{args.datadir}'
    if not args.basedir.startswith('/mnt'):
        abspath = os.path.dirname(os.path.abspath(__file__))
        args.basedir = f'{abspath}/../{args.basedir}'
    ...
    if args.dataset_type == 'llff':
        ...
        render_poses, ref_c2w, motion_coords = load_llff_data(args.datadir, 
                                                            args.start_frame-1, args.end_frame,
                                                            args.factor,
                                                            target_idx=target_idx,
                                                            recenter=True, bd_factor=.9,
                                                            spherify=False, 
                                                            final_height=args.final_height)
        novel_poses, bds = load_novel_pose(args.datadir, height=args.final_height)
        print(f"********poses shape: {poses.shape}")
        print(f"********novel_poses shape: {novel_poses.shape}")
        novel_poses = novel_poses[:,:3,:4]
    ...
    print(f"novel_poses shape: {novel_poses.shape}")
    novel_poses = torch.Tensor(novel_poses).to(device)
    ...
    with torch.no_grad():
        ...
        for img_i in i_train:
            ...
            for camera_i, view in zip([0,1,2,3,4,5,6,7,8],['21','22','23','24','25','26','27','28','29']):
                ...
                gt_img_path = sorted(glob(\
                        os.path.join(args.datadir, 'mv_images', '%02d'%int(view), '*.png')))[img_i]
                ...
                ssim = compare_ssim(gt_img, rgb, multichannel=True)
                ...
                dynamic_mask_path = sorted(glob(\
                        os.path.join(args.datadir, 'mv_masks', '%02d'%int(view), '*.png')))[img_i]    
                ...
                # save image
                imgsave_path = os.path.join(basedir, expname, 'novel_view_result', '%02d'%int(view))
                os.makedirs(imgsave_path, exist_ok=True)
                imageio.imwrite(imgsave_path + f"/{gt_img_path.split('/')[-1]}", rgb)
