...
def render_slowmo_bt(disps, render_poses, bt_poses, 
    ...
    rgbs = []
    for i, cur_time in enumerate(np.linspace(0,gt_imgs.shape[0]-1, (gt_imgs.shape[0]-1)*3 + 1).tolist()):
    # for i, cur_time in enumerate(range(0,gt_imgs.shape[0]+1)):
    ...
    # imageio.mimwrite(
    #                     savedir + "/rgb.mp4", rgbs, fps=15, quality=8
    #                 )
    # save video instead of image
    save_path = os.path.join(savedir, '../../../video', savedir.split('/')[-3])
    os.makedirs(save_path, exist_ok=True)
    save_path = os.path.join(save_path, f"{savedir.split('/')[-2]}_{savedir.split('/')[-1]}_rgb.mp4")
    imageio.mimwrite(
                        save_path, rgbs, fps=15, quality=8
                    )
...
def render_lockcam_slowmo(ref_c2w, num_img, 
    ...
    rgbs = []
    for i, cur_time in enumerate(np.linspace(0,gt_imgs.shape[0]-1, (gt_imgs.shape[0]-1)*3 + 1).tolist()):
    # for i, cur_time in enumerate(range(0,gt_imgs.shape[0]+1)): #TODO:不做时间上的插值
        ratio = cur_time - np.floor(cur_time)
