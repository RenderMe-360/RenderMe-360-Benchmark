...
void Testbed::set_camera_to_view(const Matrix<float, 3, 4>& cam, const Vector2f& focal_length, const Vector2i& resolution, const Vector4f& distortion, const Vector2f& principal_point) {
	auto old_look_at = look_at();
	m_camera = m_smoothed_camera = m_nerf.training.dataset.nerf_matrix_to_ngp(cam);
	m_relative_focal_length = focal_length / (float)resolution[m_fov_axis];
	m_scale = std::max((old_look_at - view_pos()).dot(view_dir()), 0.1f);
	m_nerf.render_with_camera_distortion = true;
	// m_nerf.render_distortion = m_nerf.training.dataset.metadata[trainview].camera_distortion;
	m_nerf.render_distortion.params[0] = distortion[0];
	m_nerf.render_distortion.params[1] = distortion[1];
	m_nerf.render_distortion.params[2] = distortion[2];
	m_nerf.render_distortion.params[3] = distortion[2];
	m_screen_center = Vector2f::Constant(1.0f) - principal_point;
}
