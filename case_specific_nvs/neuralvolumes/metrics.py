...
def masked_psnr_new(x, gt, mask):
    """
    x: np.uint8, HxWxC, 0 - 255
    gt: np.uint8, HxWxC, 0 - 255
    """
    mask_x = mask[:, :, None]
    maskL = [mask_x for i in range(x.shape[2])]
    full_mask = np.concatenate(maskL, axis=2)
    new_mask = np.array(np.where(full_mask > 0))

    x = (x / 255.).astype(np.float32)
    gt = (gt / 255.).astype(np.float32)

    mse = (((x - gt)[new_mask])** 2).mean()
    psnr = 10. * np.log10(1. / mse)
