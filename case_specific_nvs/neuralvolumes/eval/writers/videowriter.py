...
def writeimage(x):
    outpath, randid, itemnum, imgout = x
    ...
    Image.fromarray(imgout).save("{}/{}/{:06}.jpg".format(outpath, randid, itemnum))
...
class Writer():
    def __init__(self, outpath, showtarget=False, showdiff=False, bgcolor=[0., 0., 0.], colcorrect=[1.35, 1.16, 1.5], nthreads=16):
        ...
        try:
            os.makedirs("{}/{}".format(os.path.dirname(self.outpath), self.randid))
        except OSError:
            pass
    ...
    def batch(self, iternum, itemnum, irgbrec, ialpharec=None, image=None, irgbsqerr=None, **kwargs):
        ...
        if self.showdiff and irgbsqerr is not None:
        ...
        self.writepool.map(writeimage,
                zip([os.path.dirname(self.outpath) for i in range(itemnum.size(0))],
                    [self.randid for i in range(itemnum.size(0))],
                    itemnum.data.to("cpu").numpy(),
                    imgout))
        self.nitems += itemnum.size(0)

    def finalize(self, cam='testview'):
        # make video file
        video_outpath = os.path.join('/'.join(self.outpath.split('/')[:-1]), f'render_{cam}.mp4')
        command = (
                "ffmpeg -y -r 30 -i {}/{}/%06d.jpg "
                "-vframes {} "
                "-vcodec libx264 -crf 18 "
                "-pix_fmt yuv420p "
                "{}".format(os.path.dirname(self.outpath), self.randid, self.nitems, video_outpath)
                ).split()
        print(f"**video command: {command}")
        subprocess.call(command)
        if cam is None:
            shutil.rmtree("{}/{}".format(os.path.dirname(self.outpath), self.randid))
        else:
            shutil.rmtree("{}/{}".format(os.path.dirname(self.outpath), cam),ignore_errors=True)
            shutil.move("{}/{}".format(os.path.dirname(self.outpath), self.randid), "{}/{}".format(os.path.dirname(self.outpath), cam))
