    ...
    def forward(self, iternum, losslist, camrot, campos, focal, princpt, pixelcoords, validinput,
        ...
        while not done.all():
            ...
            with torch.no_grad():
                ...
                # print(f'done:{done}, t: {t}, step: {step}, tmax: {tmax}')
                done = done | ((t + step) >= tmax).type_as(done)
        ...
        if camindex is not None:
            ...
            if pixelcoords.size()[1:3] != image.size()[2:4]:
                bg = F.grid_sample(
                        torch.stack([self.bg[str(self.allcameras[camindex[i].item()])] for i in range(campos.size(0))], dim=0),
                        samplecoords)
            else:
                bg = torch.stack([self.bg[str(self.allcameras[camindex[i].item()])] for i in range(campos.size(0))], dim=0)

        rayrgb = rayrgb + (1. - rayalpha) * torch.ones_like(rayrgb) * 255.
        ...
        if image is not None:
            ...
            if "irgbmse" in losslist:
                # from pdb import set_trace as st
                # st()
                irgbmse = torch.sum(irgbsqerr.contiguous().view(irgbsqerr.size(0), -1), dim=-1)
                irgbmse_weight = torch.sum(weight.contiguous().view(weight.size(0), -1), dim=-1)
